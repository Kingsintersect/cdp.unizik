...
import { useDevSimulate, useDeclineAdmission, useAcceptAdmission, } from "../hooks/useAdmissionQueries";
...
export function AdmissionStatusSection({ student, onRefresh }: StepSectionProps) {
   ...
   const isAccepted = student.admission_status === "accepted"; // ← was never checked; step advancement depends on it
   ...
   const handleAccept = async () => {
      try {
         ...
         toast.success("Admission offer accepted! Proceeding to acceptance fee…");
         onRefresh(); // refresh parent to advance the step
      } catch {
   ...
   useEffect(() => {
      if (isDeclined || isExpired) {
         const timer = setTimeout(() => signOut({ callbackUrl: "/" }), 2500);
         return () => clearTimeout(timer);
      }
   }, [isDeclined, isExpired]);

   // Derive badge status — maps AdmissionOfferStatus to StatusType
   const admissionBadgeStatus = (() => {
      if (isPending) return "pending" as const;
      if (isOffered || isAccepted) return "success" as const;
      if (isRejected || isDeclined || isExpired) return "failed" as const;
      return "info" as const;
   })();

   const admissionBadgeLabel = (() => {
      if (isPending) return "Under Review";
      if (isOffered) return "Admission Offered!";
      ...
      return student.admission_status;
   })();
         ...
         <Card className="relative overflow-hidden border-border/50 shadow-lg">
            ...
            <CardContent className="space-y-5">
               ...
               <div className="flex flex-wrap items-center gap-2">
                  ...
                  <StatusBadgeWidget label={admissionBadgeLabel} status={admissionBadgeStatus} />
               </div>
               ...
               {isOffered && (
                  ...
                  >
                     ...
                     <div className="flex items-center gap-3">
                        <Button
                           onClick={handleAccept}               // ← was onRefresh
               ...
               {isDeclined && (
                  ...
                  >
                     ...
                     <div className="space-y-1 text-center">
                        <p className="text-sm font-semibold text-foreground">Offer Declined</p>
                        <p className="max-w-sm text-xs text-muted-foreground">
               ...
               {isExpired && (
                  ...
                  >
                     ...
                     <div className="space-y-1 text-center">
                        <p className="text-sm font-semibold text-foreground">Admission Offer Expired</p>
                        <p className="max-w-sm text-xs text-muted-foreground">
               ...
               {isRejected && (
                  ...
                  >
                     ...
                     <div className="space-y-1 text-center">
                        <p className="text-sm font-semibold text-foreground">Application Not Successful</p>
                        <p className="max-w-sm text-xs text-muted-foreground">
               ...
               {process.env.NODE_ENV === "development" && isPending && (
                  <div className="rounded-lg border border-dashed border-amber-500/30 bg-amber-500/5 p-3">
                     ...
                     <div className="flex flex-wrap gap-2">
                        ...
                        >
                           {simulateOffered.isPending ? <Loader2 className="size-3 animate-spin" /> : <CheckCircle className="size-3" />}
                  ...
                  <div className="rounded-lg border border-dashed border-amber-500/30 bg-amber-500/5 p-3">
                     ...
                     <div className="flex flex-wrap gap-2">
                        <Button
                           ...
                           className="gap-1.5 text-xs"
                        >
                           {simulateDeclined.isPending ? <Loader2 className="size-3 animate-spin" /> : <Ban className="size-3" />}
                        ...
                        <Button
                           ...
                           className="gap-1.5 text-xs"
                        >
                           {simulateExpired.isPending ? <Loader2 className="size-3 animate-spin" /> : <TimerOff className="size-3" />}
