...
export const locationService = {
    ...
    async fetchStates(): Promise<LocationItem[]> {
        try {
            const response = await apiClient.get<LocationItem[]>(`/all-states`);
    ...
    async fetchAllLocalGovAreas(): Promise<LocationItem[]> {
        try {
            const response = await apiClient.get<LocationItem[]>(`/lga`);
            if (!response.status || !response.data) {
    ...
    async fetchLocalGovAreasInState(stateId: number): Promise<LocationItem[]> {
        try {
            const response = await apiClient.get<LocationItem[]>(`lga?state_id=${stateId}`);
            if (!response.status || !response.data) {
    ...
    async fetchSingleLocalGovArea(lgaId: number): Promise<LocationItem[]> {
        try {
            const response = await apiClient.get<LocationItem[]>(`lga?state_id=${lgaId}`);
            if (!response.status || !response.data) {
